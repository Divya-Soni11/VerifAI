...
export default function ComplaintForm({ initialDarkMode = true }) {
  ...
  const handleSubmit = async () => {
    if (formData.customerID && formData.restaurantID &&
    formData.agentID &&  formData.issue && formData.description) {
     ...
     const mlResponse = {
      score: 0.78,              // 78% confidence
      status: "Under Review"
    };

    // Store ML result
    setMlScore(mlResponse.score);
    setRequestStatus(mlResponse.status);
  ...
  <div className={`mt-6 p-5 rounded-lg border ${
    ...
    <p className="text-sm mt-3 opacity-80">
      Your complaint is being verified using AI-based analysis.
    </p>
  </div>
)}
        ...
        <div className={`${
          ...
          <button
            ...
            disabled={submitted || !formData.customerID|| !formData.agentID || !formData.restaurantID || !formData.issue || !formData.description}
            className={`w-full py-3 px-4 rounded-lg font-semibold flex items-center justify-center transition-all duration-300 slide-in-right ${
              submitted || !formData.customerID|| !formData.agentID || !formData.restaurantID  || !formData.issue || !formData.description
